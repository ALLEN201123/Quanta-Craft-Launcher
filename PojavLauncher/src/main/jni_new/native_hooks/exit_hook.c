...
#include "bytehook/qcl_internal.h"
...
static void create_hooks(bytehook_hook_all_t bytehook_hook_all_p) {
    ...
    // SDL3 集成：hook SDL_Init 通知启动器初始化、窗口复用、EGL 兼容等（照搬 FCL）
...
Java_com_qcl_launcher_launcher_launch_QCLHooks_nativeInitializeHooks(JNIEnv *env, jclass clazz) {
