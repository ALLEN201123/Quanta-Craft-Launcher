...
// 1.1.0 QCL：nsbypass 可用性标志。
// 原版在初始化失败时直接 exit(121/122) 杀进程 —— 但该库只是「给 zink 加载 Turnip 驱动」
// 的可选功能，在 MuMu 等模拟器 / 部分 ROM 上 linker 内部结构不同会解析失败。
// 现在失败时安全降级：所有 nsbypass 接口返回 NULL/false，渲染回退到系统 Vulkan，
// 绝不因此崩掉整个启动器。
static bool s_nsbypass_available = false;
        ...
        private_dl_funcs privateDlFuncs)
{
    ...
    // 1.1.0 QCL：nsbypass_dlopen 可能返回 NULL（模拟器 / 非标准 linker）。
    // 原版直接把它传给 nsbypass_dlsym → 内部对 NULL 解引用 → SIGSEGV（MuMu 实测崩溃点）。
    if (linkerHandle) {
        ...
        LOGE("nsbypass_dlopen(%s) failed, cannot memory-scan linker symbols.", LINKER);
    }
...
__attribute__((constructor)) static void resolve_global_symbols()
{
    ...
    if (!s_linkerFuncs.create_namespace ||
        ...
        // 1.1.0 QCL：原版这里 exit(121) 直接杀进程。
        // 但 nsbypass 只是「给 zink 加载 Turnip 驱动」的可选功能 —— 在 MuMu 等模拟器
        // / 部分 ROM 上 linker 结构不同必然解析失败，此时应安全降级：
        // s_nsbypass_available=false → 所有接口返回 NULL/false → loadTurnipVulkan 返回 NULL
        // → 渲染自动回退到系统 Vulkan，启动器照常工作。
        LOGE("Failed to resolve Android linker namespace functions! nsbypass disabled (safe fallback, no exit).");
        s_nsbypass_available = false;
        return;
    }
    ...
    if (!escapeNs){
        ...
        if (!escapeNs) {
            // 同上：创建失败只降级，不杀进程
            LOGE("Failed to create escapeNs! nsbypass disabled (safe fallback, no exit).");
            s_nsbypass_available = false;
            return;
        }
    }
    s_nsbypass_available = true;
    LOGI("nsbypass initialized successfully.");
}
...
{
    if (is_android_6_or_lower()) return NULL;
    // 1.1.0 QCL：nsbypass 初始化失败（模拟器 / 非标准 linker）时安全返回 NULL，
    // 避免调用 NULL 函数指针崩溃。上层（loadTurnipVulkan 等）会自然回退。
    if (!s_nsbypass_available || !s_linkerFuncs.create_namespace) return NULL;
        ...
        const char* shared_libs_sonames)
{
    ...
    if (!s_nsbypass_available || !s_linkerFuncs.link_namespaces) return false;
...
{
    if (is_android_6_or_lower()) return false;
    if (!s_nsbypass_available || !s_linkerFuncs.link_namespaces_all_libs) return false;
...
{
    if (is_android_6_or_lower()) return NULL;
    if (!s_nsbypass_available || !s_linkerFuncs.get_exported_namespace) return NULL;
...
int private_dlclose(void* handle)
{
    ...
    // 1.1.0 QCL：s_privateDlFuncs 未初始化时安全返回，避免调用 NULL 函数指针
    if (!s_privateDlFuncs.dlclose) return -1;
...
{
    if (is_android_6_or_lower()) return NULL;
    if (!s_privateDlFuncs.dlopen) return NULL;
...
{
    if (is_android_6_or_lower()) return NULL;
    if (!s_privateDlFuncs.dlopen_ext) return NULL;
...
{
    if (is_android_6_or_lower()) return NULL;
    if (!s_privateDlFuncs.dlsym) return NULL;
