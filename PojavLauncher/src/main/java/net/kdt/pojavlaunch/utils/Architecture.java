...
public class Architecture {
	...
	/**
	 * Tell us the architecture the app is actually RUNNING with (from the installed ABI).
	 *
	...
	public static int getInstalledAbiArchitecture(){
		try {
			...
			// Not on Android (unit tests) or too early: fall through to the process bitness.
		}
		try {
	...
	/**
	 * Tell us the device supports 64 bits architecture
	...
	/** 运行时位数模式：0=自动、1=强制 64 位、2=强制 32 位（由启动器设置写入） */
	...
	/**
	 * 解析出**实际可用的运行时架构**：
	 * - 自动：跟随应用实际安装的 ABI（64 位设备装出来就是 arm64 → 用 64 位运行时）
	 * - 强制 64 位：只有当前进程是 64 位时才可能（32 位进程装不了 64 位 JVM）
	 * - 强制 32 位：只有当前进程是 32 位时才可能（64 位进程装不了 32 位 JVM）
	 * 无法满足时回退到应用实际 ABI，避免 JVM 全部 dlopen 失败。
	 */
