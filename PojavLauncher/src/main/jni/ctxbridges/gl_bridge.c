...
bool gl_init() {
    __android_log_print(ANDROID_LOG_ERROR, "QCL_DBG", "gl_init: enter, calling dlsym_EGL");
    if (!dlsym_EGL()) {
        __android_log_print(ANDROID_LOG_ERROR, "QCL_DBG", "gl_init: dlsym_EGL failed");
        return false;
    }
    __android_log_print(ANDROID_LOG_ERROR, "QCL_DBG", "gl_init: dlsym_EGL OK, calling eglGetDisplay_p=%p", eglGetDisplay_p);
    // ★ 1.1.0：调用前防崩检查
    if (eglGetDisplay_p == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, g_LogTag, "%s", "eglGetDisplay_p is NULL after dlsym_EGL");
        __android_log_print(ANDROID_LOG_ERROR, "QCL_DBG", "gl_init: eglGetDisplay_p is NULL, aborting");
        return false;
    }
...
gl_render_window_t *gl_init_context(gl_render_window_t *share) {
    ...
    EGLint egl_attributes[] = {EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8, EGL_ALPHA_SIZE,
                               8, EGL_DEPTH_SIZE, 24, EGL_SURFACE_TYPE,
                               EGL_WINDOW_BIT | EGL_PBUFFER_BIT, EGL_RENDERABLE_TYPE,
    ...
    if (bundle->context == EGL_NO_CONTEXT) {
        __android_log_print(ANDROID_LOG_ERROR, g_LogTag,
                            "eglCreateContext_p() finished with error: %04x",
                            eglGetError_p());
        free(bundle);
        return NULL;
    }
...
void gl_swap_surface(gl_render_window_t *bundle) {
    ...
    if ((nativeWindowWidth > 0) || (nativeWindowHeight > 0)) {
        FCL_LOG("Native surface dimensions (%d x %d)",
        ...
        FCL_LOG("Native surface dimensions (%d x %d) are invalid! Assuming android has already released window.\n",
                nativeWindowWidth, nativeWindowHeight);
    }
    if (bundle->newNativeSurface != NULL) {
        FCL_LOG("Switching to new native surface");
        ...
        __android_log_print(ANDROID_LOG_ERROR, g_LogTag,
                            "No new native surface, switching to 1x1 pbuffer");
        bundle->nativeSurface = NULL;
...
void gl_make_current(gl_render_window_t *bundle) {
    ...
    __android_log_print(ANDROID_LOG_INFO, g_LogTag,
                        "Making current, surface=%p, nativeSurface=%p, newNativeSurface=%p",
                        bundle->surface, bundle->nativeSurface, bundle->newNativeSurface);
    ...
    if (eglMakeCurrent_p(g_EglDisplay, bundle->surface, bundle->surface, bundle->context)) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, g_LogTag, "eglMakeCurrent returned with error: %04x",
                            eglGetError_p());
    }
...
void gl_swap_buffers() {
    if (currentBundle->state == STATE_RENDERER_NEW_WINDOW) {
        eglMakeCurrent_p(g_EglDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE,
                         EGL_NO_CONTEXT); //detach everything to destroy the old EGLSurface
    ...
    if (currentBundle->surface != NULL)
        if (!eglSwapBuffers_p(g_EglDisplay, currentBundle->surface) &&
            ...
            eglMakeCurrent_p(g_EglDisplay, currentBundle->surface, currentBundle->surface,
                             currentBundle->context);
            __android_log_print(ANDROID_LOG_INFO, g_LogTag,
                                "The window has died, awaiting window change");
        }
...
Java_org_lwjgl_opengl_PojavRendererInit_nativeInitGl4esInternals(JNIEnv *env, jclass clazz,
    ...
    jmethodID method_getFunctionAddress = (*env)->GetMethodID(env, funcProviderClass,
                                                              "getFunctionAddress",
...
#define GETSYM(N) ((*env)->CallLongMethod(env, function_provider, method_getFunctionAddress, (*env)->NewStringUTF(env, N)));
    void (*set_getmainfbsize)(void (*new_getMainFBSize)(int *width, int *height)) = (void *) GETSYM(
            "set_getmainfbsize");
    if (set_getmainfbsize != NULL) {
        __android_log_print(ANDROID_LOG_INFO, g_LogTag,
                            "GL4ES internals initialized dimension callback");
