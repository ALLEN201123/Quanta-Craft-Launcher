...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    // The expected identity belongs to upstream HMCL-PE. QCL is a separate product with its own
    // applicationId, so accept both and only enforce the check on non-public builds.
    bool packageAccepted = strcmp(package_name, app_packageName) == 0 ||
                           strcmp(package_name, "com.qcl.launcher") == 0;
    if (!packageAccepted && !isPublic) {
        abort();
    }
    LOGE("package:%s hashcode:%ld", package_name, hashCode);
