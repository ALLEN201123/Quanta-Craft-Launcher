...
public class Avatar {
    ...
    /** 直接从皮肤位图裁出头像（脸部 8x8 + 帽层 8x8）：离线账号的头像跟随皮肤。 */
    public static void setAvatarFromSkin(final Bitmap skin, final ImageView face, final ImageView hat){
        ...
        face.post(() -> {
            try {
                ...
                Matrix matrix = new Matrix();
                matrix.postScale(face.getWidth() / 8f, face.getHeight() / 8f);
                Matrix matrixSec = new Matrix();
                matrixSec.postScale(hat.getWidth() / 8f, hat.getHeight() / 8f);
                face.setImageBitmap(Bitmap.createBitmap(faceBitmap, 0, 0, 8, 8, matrix, false));
                hat.setImageBitmap(Bitmap.createBitmap(hatBitmap, 0, 0, 8, 8, matrixSec, false));
            } catch (Throwable t) {
                t.printStackTrace();
            }
        });
