...
public class SettingUtils {
    ...
    public static ArrayList<GameListBean> getLocalVersionInfo(String path,String currentVersion){
        ...
        // list() returns null when the directory does not exist yet (fresh install / missing
        // game directory) -- iterating that null used to take the version list down.
        String[] string = versionsDir.list();
        if (string == null){
            return list;
        }
        if (versionsDir.exists()){
            for (String str : string){
                if (new File(path + "/versions/" + str + "/" + str + ".json").exists() && new File(path + "/versions/" + str + "/" + str + ".jar").exists()) {
                    ...
                    if (version == null) {
                        // 玩家手拷进来的"诡异版本"（只有 jar + json，json 可能不合规范）。
                        // 不能藏：照样列出来，版本号先用文件夹名顶上，
                        // 启动时的 CheckLibTask 会再检查并补齐缺的依赖。
                        Log.e("QCL", "版本 json 无法解析，仍按文件夹名显示: " + str);
                        bean.version = str;
