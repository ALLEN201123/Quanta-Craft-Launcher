        ...
        Thread.setDefaultUncaughtExceptionHandler((thread, error) -> {
            ...
            try {
                ...
                android.content.Intent intent = new android.content.Intent(HMCLPEApplication.this,
                        com.qcl.launcher.launcher.CrashReportActivity.class);
                intent.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK
                        | android.content.Intent.FLAG_ACTIVITY_CLEAR_TOP);
                intent.putExtra(com.qcl.launcher.launcher.CrashReportActivity.EXTRA_CRASH_TEXT, buffer.toString());
                intent.putExtra(com.qcl.launcher.launcher.CrashReportActivity.EXTRA_SUMMARY,
                        error.getClass().getName() + ": " + String.valueOf(error.getMessage()));
                startActivity(intent);
                // 杀掉崩溃的主进程（崩溃界面跑在独立进程 :crash，不受影响）。
                // 否则主进程处于半死状态，「返回启动器」重启 MainActivity 会因主线程 Looper 已坏而再次崩溃。
