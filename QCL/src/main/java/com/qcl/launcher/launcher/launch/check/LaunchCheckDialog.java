...
import com.qcl.launcher.launcher.launch.boat.BoatMinecraftActivity;
import com.qcl.launcher.launcher.launch.boat.VirGLService;
    ...
    private void startCheckTasks() {
        checkJavaTask = new CheckJavaTask(activity, launchVersion, new CheckJavaTask.CheckJavaCallback() {
            ...
            public void onFinish(Exception e) {
                if (e == null) {
                    java = true;
                    javaState.setBackground(getContext().getDrawable(R.drawable.ic_baseline_done_white));
                    checkState();
                }
                else {
                    throwException(e);
        ...
        checkLibTask = new CheckLibTask(activity, launchVersion, new CheckLibTask.CheckLibCallback() {
            ...
            public void onFinish(Exception e) {
                if (e == null) {
                    lib = true;
                    libState.setBackground(getContext().getDrawable(R.drawable.ic_baseline_done_white));
                    checkState();
                }
                else {
                    throwException(e);
        ...
        checkAccountTask = new CheckAccountTask(activity, new CheckAccountTask.CheckAccountCallback() {
            ...
            public void onFinish(Exception e, boolean finish) {
                if (finish) {
                    if (e == null) {
                        login = true;
                        loginState.setBackground(getContext().getDrawable(R.drawable.ic_baseline_done_white));
                        checkState();
                    }
                    else {
                        throwException(e);
        ...
        if (java && lib && login) {
            launchTask = new LaunchTask(activity, new LaunchTask.LaunchCallback() {
                ...
                public void onFinish(Vector<String> args) {
                    launchState.setBackground(getContext().getDrawable(R.drawable.ic_baseline_done_white));
